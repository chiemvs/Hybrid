...
leadtimepool = [15,16,17,18,19,20,21] # From the longest leadtimepool is taken
target_region = 10
...
predictors, forc, obs = prepare_full_set(targetname, ndaythreshold = 9, predictand_cluster = target_region, leadtimepool = leadtimepool)
...
continuous_obs = read_raw_predictand(continuous_tg_name, clustid = target_region, separation = leadtimepool)
...
jfilter_det = filter_predictor_set(X_trainval.reindex(continuous_obs.index), detrended_exceedence, return_measures = False, nmost_important = 10, nbins=10)
